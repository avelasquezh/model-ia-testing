...
<html>
  <body>
    <main>
      <h1>F2-42 Controlled chatbot</h1>
...
test('F2-42: empirically validates repeatable observable evidence for selected criteria', async () => {
  ...
  try {
    const observations: Array<{ response: string; durationMs: number; screenshotBytes: number }> = [];
    ...
    expect(observations.every((item) => item.screenshotBytes > 0)).toBe(true);

    // Methodological result only: D1 and D7 are reproducibly observable and D6 is measurable.
    expect(JSON.stringify(observations)).not.toContain('qualityScore');
    expect(JSON.stringify(observations)).not.toContain('REGRESSION');
    expect(JSON.stringify(observations)).not.toContain('IMPROVEMENT');
  } finally {
