...
test('F2-19: executes a tangible chatbot scenario and captures observable evidence', async () => {
  ...
  const scenario = new Scenario({
    ...
    objective: 'Verify a user message receives an observable response',
    description: 'Controlled browser chatbot used as an executable evaluation target.',
    inputs: [{ value: 'Quiero una camisa azul talla M' }],
    expectedBehavior: 'The chatbot displays an observable response containing the submitted message.',
    finishConditions: [{ description: 'A bot response is visible in the conversation.' }],
    version: 1,
  });
  const execution = new Execution({
    id: 'execution-f2-19-001',
