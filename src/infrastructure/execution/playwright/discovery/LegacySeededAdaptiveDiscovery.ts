...
type SeedHandle = { readonly source: ChatDiscoveryCandidate; readonly locator: Locator; readonly context: Page | Frame; readonly candidate: AdaptiveCandidateScore; readonly key: string };
export type LegacySeededAdaptiveDiscoveryOptions = { readonly maxSeeds?: number; readonly maxClicks?: number; readonly settleMs?: number };
export type LegacySeededAdaptiveDiscoveryRun = { readonly model: 'LEGACY_SEEDED_ADAPTIVE'; readonly legacy: ChatDiscoveryReport; readonly seedsConsidered: number; readonly clicksAttempted: number; readonly experiments: readonly DiscoveryExperimentResult[]; readonly selected?: DiscoveryExperimentResult; readonly debug: readonly AdaptiveDiscoveryDebugAttempt[] };

const DEFAULT_MAX_SEEDS = 40;
const DEFAULT_MAX_CLICKS = 12;
const DEFAULT_SETTLE_MS = 350;

/** Independent third discovery model. It does not modify LEGACY or ADAPTIVE. */
export class LegacySeededAdaptiveDiscovery {
  ...
  public async run(): Promise<LegacySeededAdaptiveDiscoveryRun> {
    ...
    if (targetUrl !== 'about:blank') await this.restore(targetUrl, debug);
    ...
    for (const seed of seeds) {
      ...
      const result: DiscoveryExperimentResult = { candidate: seed.candidate, before, after, diff, classification: classifyExperiment(diff) };
      experiments.push(result);
      if (result.classification === 'CHAT_SURFACE_CANDIDATE') return { model: 'LEGACY_SEEDED_ADAPTIVE', legacy, seedsConsidered: seeds.length, clicksAttempted, experiments, selected: result, debug };
      if (beforeUrl !== 'about:blank') await this.restore(beforeUrl, debug);
    }
  ...
  private async discoverLegacy(debug: AdaptiveDiscoveryDebugAttempt[]): Promise<ChatDiscoveryReport> {
    try { const result = await new PlaywrightChatDiscovery(this.page).discoverWithEvidence(); return result.report; }
    catch (error) { if (error instanceof ChatDiscoveryError && error.report) return error.report; debug.push({ candidateId: 'legacy', score: 0, frameUrl: this.page.url(), stage: 'COLLECT', action: 'INSPECT', ok: false, error: this.errorMessage(error) }); throw error; }
  }

  private async collectLegacySeeds(report: ChatDiscoveryReport, debug: AdaptiveDiscoveryDebugAttempt[]): Promise<SeedHandle[]> {
    const handles: SeedHandle[] = []; const seen = new Set<string>(); const maxSeeds = this.options.maxSeeds ?? DEFAULT_MAX_SEEDS;
    const matched = report.candidates.filter((candidate) => candidate.matched && candidate.element && candidate.role === 'launcher');
    for (const source of matched) {
      const element = source.element; if (!element) continue;
      ...
      for (const context of contexts) {
        ...
        const locator = this.locatorForEvidence(context, element); if (!locator) continue;
        const candidate = await this.buildCandidate(locator); if (!candidate) continue;
        const key = this.candidateKey(candidate); if (seen.has(key)) continue; seen.add(key);
        const scored = scoreDiscoveryCandidate(candidate); handles.push({ source, locator, context, candidate: scored, key });
        debug.push({ candidateId: scored.candidateId, score: scored.score, frameUrl: context.url(), stage: 'COLLECT', action: 'INSPECT', ok: true }); break;
      }
  ...
  private async buildCandidate(locator: Locator): Promise<DiscoveryCandidate | null> {
    ...
    const box = await locator.boundingBox().catch(() => null); if (!box) return null;
    ...
    return { id: `${tagName}:${ariaLabel ?? text}:${Math.round(box.x)}:${Math.round(box.y)}`, tagName, ...(role ? { role } : {}), ...(ariaLabel ? { ariaLabel } : {}), text, ...(placeholder ? { placeholder } : {}), ...(href ? { href } : {}), ...(title ? { title } : {}), ...(name ? { name } : {}), ...(testId ? { testId } : {}), ...(ariaControls ? { ariaControls } : {}), ...(typeof ariaExpanded === 'string' ? { ariaExpanded } : {}), ...(navigationSignal ? { navigationSignal } : {}), fixed, ...(viewport ? { bottomDistance: Math.max(0, viewport.height - (box.y + box.height)), rightDistance: Math.max(0, viewport.width - (box.x + box.width)) } : {}) };
  }

  private candidateKey(candidate: DiscoveryCandidate): string { return `${candidate.tagName}|${candidate.role ?? ''}|${candidate.ariaLabel ?? ''}|${candidate.text ?? ''}|${candidate.testId ?? ''}|${candidate.href ?? ''}`; }
  private async safeClick(seed: SeedHandle, debug: AdaptiveDiscoveryDebugAttempt[]): Promise<boolean> { try { await seed.locator.scrollIntoViewIfNeeded({ timeout: 1_500 }); await seed.locator.click({ timeout: 3_000, noWaitAfter: true }); debug.push({ candidateId: seed.candidate.candidateId, score: seed.candidate.score, frameUrl: seed.context.url(), stage: 'CLICK', action: 'CLICK', ok: true }); return true; } catch (error) { debug.push({ candidateId: seed.candidate.candidateId, score: seed.candidate.score, frameUrl: seed.context.url(), stage: 'CLICK', action: 'CLICK', ok: false, error: this.errorMessage(error) }); return false; } }
  private async restore(url: string, debug: AdaptiveDiscoveryDebugAttempt[]): Promise<void> { try { await this.page.goto(url, { waitUntil: 'domcontentloaded', timeout: 8_000 }); } catch (error) { debug.push({ candidateId: 'restore', score: 0, frameUrl: this.page.url(), stage: 'RESTORE', action: 'RESTORE', ok: false, error: this.errorMessage(error) }); } }

  private async snapshot(debug: AdaptiveDiscoveryDebugAttempt[], stage: 'SNAPSHOT_BEFORE' | 'SNAPSHOT_AFTER'): Promise<UiSnapshot> {
    const contexts: Array<Page | Frame> = [this.page, ...this.page.frames().filter((frame) => frame !== this.page.mainFrame())];
    const parts: string[] = [];
    let visibleElementCount = 0, dialogCount = 0, textboxCount = 0, formCount = 0, iframeCount = 0;
    let contentEditableCount = 0, liveRegionCount = 0, messageNodeCount = 0, chatSignalCount = 0, shadowRootCount = 0;
    for (const context of contexts) {
      try {
        const html = await context.locator('html').evaluate((element) => element.outerHTML); parts.push(`${context.url()}|${html}`);
        const counts = await context.locator('body').evaluate((body) => {
          const elements = Array.from(body.querySelectorAll('*'));
          let visibleElementCount = 0, dialogCount = 0, textboxCount = 0, formCount = 0, iframeCount = 0;
          let contentEditableCount = 0, liveRegionCount = 0, messageNodeCount = 0, chatSignalCount = 0, shadowRootCount = 0;
          const chatTerms = /chat|livechat|chatbox|messenger|message|messages|conversation|conversacion|support|help|assistant|contact|customer|bot|ayuda|asistente|servicio/i;
          const messageTerms = /message|messages|response|reply|conversation|mensaje|respuesta|conversacion/i;
          for (const element of elements) {
            ...
            if (element.matches('[contenteditable="true"]')) contentEditableCount += 1;
            if (element.matches('[aria-live]')) liveRegionCount += 1;
            const semantic = `${element.getAttribute('aria-label') ?? ''} ${element.getAttribute('title') ?? ''} ${element.getAttribute('data-testid') ?? ''} ${element.id} ${element.className ?? ''} ${element.textContent?.slice(0, 160) ?? ''}`;
            if (messageTerms.test(semantic)) messageNodeCount += 1;
            if (chatTerms.test(semantic)) chatSignalCount += 1;
            if (element.shadowRoot) shadowRootCount += 1;
          }
          return { visibleElementCount, dialogCount, textboxCount, formCount, iframeCount, contentEditableCount, liveRegionCount, messageNodeCount, chatSignalCount, shadowRootCount };
        });
        ...
        contentEditableCount += counts.contentEditableCount; liveRegionCount += counts.liveRegionCount; messageNodeCount += counts.messageNodeCount; chatSignalCount += counts.chatSignalCount; shadowRootCount += counts.shadowRootCount;
      } catch (error) { debug.push({ candidateId: `context:${context.url()}`, score: 0, frameUrl: context.url(), stage, action: 'INSPECT', ok: false, error: this.errorMessage(error) }); }
    }
    return { domHash: createHash('sha256').update(parts.join('\n')).digest('hex'), visibleElementCount, dialogCount, textboxCount, formCount, iframeCount, contentEditableCount, liveRegionCount, messageNodeCount, chatSignalCount, shadowRootCount };
  }
