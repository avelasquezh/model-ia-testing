...
async function runTarget(browser: Browser, target: Target): Promise<TargetResult> {
  ...
  for (const stage of stages) {
    ...
    try {
      ...
      const run: AdaptiveDiscoveryRun = await new AdaptiveDiscoveryExperimentRunner(page, { maxCandidates, maxClicks: stage.maxClicks, settleMs }).run();
      stageResults.push({ stage: stage.name, ok: true, durationMs: Date.now() - started, candidatesConsidered: run.candidatesConsidered, clicksAttempted: run.clicksAttempted, experiments: run.experiments.length, chatSurfaceFound: Boolean(run.selected), debugEvents: run.debug.length });
    } catch (error) {
